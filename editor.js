function onFrozen(elem, idx) {
	...
	$img = $div.find("img");
	if (wasFrozen) {
		elem.title = "Freeze field";
		$img.attr("src", src_unfrozen);
	} else {
		elem.title = "Unfreeze field";
...
function setFrozenFields(fields, frozen) {
	...
	for (var i = 0; i < fields.length; i++) {
		...
		if (frozen[i]) {
			txt += "<td style='width:28px'><div id=i" + i + " title='Unfreeze field'" + " onclick='onFrozen(this, " + i + ");'><img src='" + src_frozen + "'/></div></td>";
		}
		else {
			txt += "<td style='width:28px'><div id=i" + i + " title='Freeze field' onclick='onFrozen(this, " + i + ");'><img src='" + src_unfrozen + "'/></div></td>";
		}
